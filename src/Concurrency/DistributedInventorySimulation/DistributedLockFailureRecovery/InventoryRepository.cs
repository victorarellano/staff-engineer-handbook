...
public sealed class InventoryRepository
{
    ...
    public async Task<bool> TryPurchaseAsync(
        string instanceName, int productId, int quantity, int operationDelayMilliseconds, int lockTimeoutMilliseconds, int lockRetryDelayMilliseconds, bool simulateFailure, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (simulateFailure)
            {
                Console.WriteLine($"{instanceName} simulating failure while holding lock.");
                throw new InvalidOperationException("Simulated failure inside critical section.");
            }
